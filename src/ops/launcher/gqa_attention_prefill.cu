...
void gqa_attention_prompt_attention_launch_for(const Tensor& q, const Tensor& positions,
    ...
    static const cudaError_t attr_i8 =
        cudaFuncSetAttribute(gqa_attention_prefill_i8_kernel<Geometry, false, false, false, false>,
                             cudaFuncAttributeMaxDynamicSharedMemorySize, kGqaPrefillI8SmemBytes);
    CUDA_CHECK(attr_i8);
    static const cudaError_t attr_i4 =
        cudaFuncSetAttribute(gqa_attention_prefill_i8_kernel<Geometry, true, true, true, false>,
                             cudaFuncAttributeMaxDynamicSharedMemorySize, kGqaPrefillI8SmemBytes);
    CUDA_CHECK(attr_i4);
    static const cudaError_t attr_k8v4 =
        cudaFuncSetAttribute(gqa_attention_prefill_i8_kernel<Geometry, false, true, true, false>,
                             cudaFuncAttributeMaxDynamicSharedMemorySize, kGqaPrefillI8SmemBytes);
    CUDA_CHECK(attr_k8v4);
    static const cudaError_t attr_rk8v4 =
        cudaFuncSetAttribute(gqa_attention_prefill_i8_kernel<Geometry, false, true, true, true>,
                             cudaFuncAttributeMaxDynamicSharedMemorySize, kGqaPrefillI8SmemBytes);
    CUDA_CHECK(attr_rk8v4);
    ...
    if (cache.dtype == DType::I8 || cache.dtype == DType::U8) {
        ...
        auto launch = [&]<bool PackedK, bool PackedV, bool RotateK, bool RotateV>() {
            gqa_attention_prefill_i8_kernel<Geometry, PackedK, PackedV, RotateK, RotateV>
        ...
        if (cache.dtype == DType::U8) {
            ...
            if (cache.rotate_v) {
                launch.template operator()<false, true, true, true>();
            } else {
                launch.template operator()<false, true, true, false>();
            }
        } else {
            launch.template operator()<false, false, false, false>();
        }
        if (cache.rotate_v) {
            constexpr int kGroups = kGqaPrefillHeadDim / kGqaKvQuantGroup;
            const int units       = tokens * Geometry::QHeads * kGroups;
            gqa_kv_inverse_rotate_output_kernel<Geometry::QHeads>
                <<<units, 32, 0, stream>>>(static_cast<__nv_bfloat16*>(out.data), tokens);
        }
    } else {
...
void gqa_kv_append_launch_for(const Tensor& k, const Tensor& v, const Tensor& positions,
    ...
    if (cache.dtype == DType::I8 || cache.dtype == DType::U8) {
        ...
        auto launch = [&]<bool PackedK, bool PackedV, bool RotateK, bool RotateV>() {
            gqa_attention_prefill_fill_i8_kernel<Geometry, PackedK, PackedV, RotateK, RotateV>
