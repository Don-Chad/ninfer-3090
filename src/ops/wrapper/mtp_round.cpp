...
void mtp_prepare_next_round(const Tensor& verify_ids, const Tensor& next_anchors,
    ...
    const std::int32_t verify_window = T - 1;
    if (proposal_depth < 1 || proposal_depth > verify_window || proposal_depth > 5) {
        throw std::invalid_argument(
            "mtp_prepare_next_round: proposal depth must be in [1,V]");
    }
    ...
    const std::int32_t steps = std::max(proposal_depth - 1, 1);
    ...
    detail::mtp_prepare_next_round_launch(verify_ids, next_anchors, accepted, updated_frontiers,
                                          ...
                                          ar_rope_positions, ar_valid_columns, proposal_depth,
                                          max_context, stream);
}
