...
void causal_attention_small_t_launch(
    ...
    if (cache.packed_v) {
        gqa_attention_small_t_launch(q, k, v, pos, valid_columns, table_rows, scale, cache,
...
void causal_attention_cached_small_t_launch(const Tensor& q, const Tensor& pos, float scale,
    ...
    if (cache.packed_v) {
        gqa_attention_cached_small_t_launch(q, pos, scale, cache, envelope, partial_acc, partial_m,
                                            partial_l, out, stream);
        return;
    }
    if (cache.dtype == DType::FP8_E4M3FN) {
