...
__global__ void mtp_prepare_next_round_kernel(
    ...
    std::int32_t* ar_rope_positions, std::int32_t* ar_valid_columns,
    std::int32_t verify_window, std::int32_t proposal_depth, std::int32_t ar_step_stride,
    std::int32_t max_context) {
    const int row = static_cast<int>(blockIdx.y);
    const int T   = verify_window + 1;
    int a         = accepted[row];
    a             = a < 0 ? 0 : (a > verify_window ? verify_window : a);
    ...
    if (blockIdx.x == 0 && threadIdx.x == 0) {
        ...
        next = next < 0 ? 0 : (next > proposal_depth ? proposal_depth : next);
        next_extents[row]        = next;
        const int steps          = proposal_depth > 1 ? proposal_depth - 1 : 1;
