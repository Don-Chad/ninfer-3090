...
__global__ void linear_rowsplit_gemv_attn_kv_1024_q4_kernel(
...
__global__ void linear_rowsplit_gemv_attn_kv_1024_q5_kernel(
    const __nv_bfloat16* __restrict__ x, const std::uint8_t* __restrict__ codes,
    const std::uint8_t* __restrict__ scales, __nv_bfloat16* __restrict__ out) {
    const int lane = static_cast<int>(threadIdx.x) & 31;
    const int warp = static_cast<int>(threadIdx.x) >> 5;
...
void linear_rowsplit_gemv_attn_kv_1024_q4_launch(const Tensor& x, const Weight& w, Tensor& out,
    ...
    if (w.n != kN || w.k != kK || w.padded_shape[1] != kK) {
        throw std::invalid_argument("linear: AttnKV1024 Q4 tuned GEMV requires 1024x5120");
    }
    linear_rowsplit_gemv_attn_kv_1024_q4_kernel<<<kN, kQ4BlockThreads, 0, stream>>>(
...
void linear_rowsplit_gemv_attn_kv_1024_q5_launch(const Tensor& x, const Weight& w, Tensor& out,
                                                 cudaStream_t stream) {
    if (w.n != kN || w.k != kK || w.padded_shape[1] != kK) {
        throw std::invalid_argument("linear: AttnKV1024 Q5 tuned GEMV requires 1024x5120");
    }
    linear_rowsplit_gemv_attn_kv_1024_q5_kernel<<<kN, kQ5BlockThreads, 0, stream>>>(
        static_cast<const __nv_bfloat16*>(x.data), static_cast<const std::uint8_t*>(w.qdata),
        static_cast<const std::uint8_t*>(w.scales), static_cast<__nv_bfloat16*>(out.data));
    CUDA_CHECK(cudaGetLastError());
}
