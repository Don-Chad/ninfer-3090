...
__global__ void linear_rowsplit_gemv_out_6144_q5_kernel(const __nv_bfloat16* __restrict__ x,
    ...
    float acc = 0.0f;
#pragma unroll 1
    for (int group = 0; group < kGroups; group += 3) {
        ...
        } else if (lane < 3 * kWordsPerGroup) {
            lane_word = *reinterpret_cast<const std::uint32_t*>(
                code_group2 + (lane - 2 * kWordsPerGroup) * 4);
        }
        ...
        acc = accumulate_group(x2, lane_word, scale2_bits, 2 * kWordsPerGroup, lane, group + 2,
                               acc);
    }
...
void linear_rowsplit_gemv_out_6144_q5_launch(const Tensor& x, const Weight& w, Tensor& out,
    ...
    linear_rowsplit_gemv_out_6144_q5_kernel<<<grid, kBlockThreads, 0, stream>>>(
