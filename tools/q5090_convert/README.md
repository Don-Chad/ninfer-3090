...
-> a single self-describing packed file in the `q5090_w4g64_mixed_v4` format.

- Binary file contract (self-contained v4, incl. the optional draft `lm_head`):
...
The output **always contains all three modules** (TEXT_CORE + MTP_DRAFT + VISION_ENCODER) so
it never needs regenerating; the runtime skips the modules it does not need. There are no runtime
transforms: the converter may canonicalize documented TEXT_CORE shapes into q5090 runtime-native form.

Unless `--no-draft-head` is given, TEXT_CORE also embeds an optional shortlisted draft `lm_head`
...
| `tensor_plan.py` | declarative source->qtype/layout/slice tables for all three modules |
...
| `verify.py` | L0 structure/plan checks, L1 bit-exact quantizer checks, and `conv_dump.v4.json` |
...
/home/neroued/miniconda3/envs/py311/bin/python -m tools.q5090_convert.convert \
  ...
  --out out/qwen3_6_27b.q5090_w4g64_mixed_v4.qus

/home/neroued/miniconda3/envs/py311/bin/python -m tools.q5090_convert.verify \
  out/qwen3_6_27b.q5090_w4g64_mixed_v4.qus
```

The converter writes `<weights>.manifest.json` next to the packed file. The verifier writes
`out/conv_dump.v4.json` unless `--dump FILE` is supplied. Useful flags: `--out FILE`, `--device cpu`,
`--force` (config mismatch -> warn), `--no-draft-head`, `--ranking FILE`, `--draft-n N`, and
`--tokenizer DIR`. The verifier re-derives the shortlist for L1 (`--ranking`/`--tokenizer`) and
