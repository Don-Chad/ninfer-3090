 ...
 * logical dequantized matrix product and its BF16 output boundary. Q4 RowSplit execution is
 * registered for a finite set of exact physical problems; the encoding and alignment contract
 * alone do not imply arbitrary-shape support. `out` must not overlap x or any weight plane. `ws`
 * supplies transient policy scratch and carries no semantic state beyond the call.
 */
