 ...
 * For query head h, kvh=floor(h/6), p=positions[t], and a populated cache history [0,p]:
 *
 ...
 *
 * q/out are contiguous BF16 [256,24,T], k/v are contiguous BF16 [256,4,T], and positions is
 * contiguous I32 [T]. `scale` is 1/sqrt(256). Positions are valid cache slots in causal order and
 * all earlier slots read by attention are populated. The selected cache layer is either BF16
 * [256,padded_context,4] or group-64 I8 codes with FP16 scales [4,padded_context,4]. The logical
 * formula uses the values represented by that cache format.
 *
 ...
 * `ws` provides transient capacity reported by gqa_attention_workspace_bytes and is scoped to the
 * call.
 */
...
/**
 * Performs only the cache-write part of gqa_attention. k/v are contiguous BF16 [256,4,T] and
 * positions is contiguous I32 [T]; cache format, position, alias, and `kv.pos` semantics are the
 * same as above. Every addressed K/V code (and I8 scale) is overwritten. No workspace is used.
 */
 ...
 * q/out are contiguous BF16 [256,24,T], positions is contiguous I32 [T], and scale is
 * 1/sqrt(256). It writes all of out, does not mutate the cache or `kv.pos`, and uses no caller
 * workspace. Inputs, output, and cache storage must not overlap.
 */
