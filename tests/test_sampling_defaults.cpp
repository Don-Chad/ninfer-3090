...
int main() {
    ...
    const ninfer::ModelSamplingDefaults huihui_qwen3_8 =
        Dense27::sampling_defaults(Dense27::huihui_qwen3_8_abliterated_model_id);
    ...
    failures += check(same_preset(huihui_qwen3_8.thinking, dense_thinking) &&
                          same_preset(huihui_qwen3_8.non_thinking, dense_non_thinking),
                      "Huihui Qwen3.8-27B abliterated defaults mismatch");
    failures += check(
        Dense27::resolve_weights(
            {std::string(Dense27::huihui_qwen3_8_abliterated_model_id), "groupwise-int"}) ==
            Dense27::WeightsProfile::GroupwiseIntW8Endpoints,
        "Huihui Qwen3.8-27B abliterated identity resolved to the wrong weights profile");
