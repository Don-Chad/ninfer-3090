...
static std::size_t idx2(std::int32_t c, std::int32_t t, std::int32_t C) {
    return static_cast<std::size_t>(t) * static_cast<std::size_t>(C) +
...
static void cpu_prefill_ref(const std::vector<float>& x, const std::vector<float>& weight,
                            ...
                            std::int32_t T, std::vector<double>& out,
    ...
    for (std::int32_t s = 0; s < 3; ++s) {
        ...
        for (std::int32_t c = 0; c < C; ++c) {
            final_state[idx2(c, s, C)] = (seq_pos < 3)
                ? static_cast<double>(initial_state[idx2(c, seq_pos, C)])
...
static int one_prefill_shape(const char* tag, std::int32_t C, std::int32_t T,
                             std::uint32_t seed, float lo = -8.f, float hi = 8.f) {
    ...
    f += verify((std::string(tag) + " state").c_str(), from_device_bf16(dstate, state_n),
                ref_state, Tolerance::bf16_reduction());
    return f;
}

static int one_decode_shape(const char* tag, std::int32_t C, std::uint32_t seed,
...
int main() {
    ...
    for (std::uint32_t seed : {1u, 7u, 99u}) {
        ...
        f += one_prefill_shape("causal_conv1d prefill [10241,64]", 10241, 64,
                               seed + 4000u);
        f += decode_chain_equivalence(seed + 5000u);
    }
    f += one_prefill_shape("causal_conv1d stress prefill [10241,64] [-32,32]", 10241, 64,
                           4242u, -32.f, 32.f);
    f += one_decode_shape("causal_conv1d stress decode [10240,1] [-32,32]", 10240, 4343u,
                          -32.f, 32.f);
