...
- runtime 必须对实际 asset bytes 重算 CRC32 和 SHA-256，并验证 canonical SHA、UTF-8、
  JSON/merges 语义和全部 tokenizer padding；只比较 record 中声明的 SHA 字段不构成 identity 绑定；
...
- 三份 asset 的 SHA-256 固定为规范 §2.1 的 canonical 值，converter 不接受与权重目录分离的
  tokenizer override；occupied id 必须恰好为连续的 `[0,248077)`，从而绑定 token/id 与权重行；
- full head 的 `[248077,248320)` 保留行在 argmax/sampling 前统一写为 `-inf`，draft id-map 也只能引用
  `[0,248077)`；
...
- `EMBED`/full `LM_HEAD` 的逻辑 shape 必须等于 `[vocab_size, hidden_size]`，以上校验均在 CUDA
  context 创建前完成；
...
- id-map 中每个 id 唯一且 `< 248077`（canonical occupied tokenizer domain）；
...
3. `TEXT_CORE` 不含 draft source kinds；`LM_HEAD_DRAFT` 严格满足 §5.3 两-block 合同。
   TEXT_CORE 还必须在 CPU preflight 校验完整的 819 blocks / 963 segments / 128 fusion groups，
   覆盖 final norm、64 层 full/GDN/MLP 的 name/qtype/shape/segment/fusion 合同，不能推迟到 H2D 后 bind。
...
5. runtime 读取 tokenizer 后强制 CRC/UTF-8/内容解析；offline verifier 额外强制 SHA-256 和所有
   ...
   内容解析包括 §4.3 的 token-id/weight-vocab 交叉校验，不允许把 tokenizer 构造推迟到 H2D 后。
...
converter 只从 `--model` 目录读取 tokenizer 与 `tokenizer_config.json`，三份 runtime asset 必须全部
存在且命中 frozen SHA-256；不提供独立 `--tokenizer` override，避免 token/id 与权重行静默错配。
...
5. `qus`、serve、benchmark 和其他所有入口只接收一个 `.qus`；Engine 路径统一在 CUDA 前构造
   tokenizer（`qus_bench` 也走该路径），直接使用 WeightStore 的算子 benchmark 只解析 raw bundle、
   不构造派生表；
...
`WeightStore` 是 one-shot resident owner；替换权重必须先显式 `clear()`，本阶段不支持 hot-load。因此
所有已发布 descriptor address 和 device pointer 在 Engine 生命周期内稳定。
...
callback 或 wait 异常都先 drain stream；无法确认 stream quiescent 时按 CUDA-fatal 终止。
progress callback 内的 `clear()` 延迟到当前 transaction 安全退出后执行；嵌套
`prepare()`/`upload()`/`load()` 立即拒绝，禁止 callback 重入使 plan、artifact 或 arena 引用失效。
I/O/H2D byte counters 表示最近一次 attempt，并在每次成功 `pread`/enqueue 后增量记账，因此失败路径也
保留已完成工作量。
