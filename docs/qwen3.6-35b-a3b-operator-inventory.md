...
qualified specialized decode kernels, not a generic Linear fallback. Its availability is recorded
below, but it does not relax this support definition or the grouped Small-T/prefill requirements.
`SparseMoe` itself accepts any positive T; target schedule bounds do not become Op bounds.
...
|---|---|---|---|---|
| L15 | Private Linear; BF16 weight `[257,2048]`, BF16 `X[2048,T]` -> logical scores `[257,T]` | All three T regimes | Rows `0:256` are router logits and row 256 is the independent shared-expert score. | **The single-column kernel is performance-qualified; every positive T is functional.** The 257-CTA, 8-warp kernel writes private FP32 scores and sits on its same-grid payload floor. The current implementation applies it independently to every column; grouped performance remains unsupported. See the [retained decode report](archive/optimization-era/bench/qwen3.6-35b-sparse-moe-decode-roofline.md). |
| L16 | Private LinearSwiGLU; W8 weight `[1024,2048]`, BF16 `X[2048,T]` -> logical activation `[512,T]` | All three T regimes | Split `W X` into gate/up `[512,T]`; return `SiLU(gate).*up` for the always-on shared expert. | **The single-column kernel is performance-qualified; every positive T is functional.** D3 computes the shared W8 pair with the selected routed experts and retains its natural FP32 result. Column iteration is correct but does not qualify grouped weight reuse. |
| L17 | Private Linear; W8 weight `[2048,512]`, logical `X[512,T]` -> logical result `[2048,T]` | All three T regimes | `Y_shared=W_shared_down X`. | **The single-column kernel is performance-qualified; every positive T is functional.** The exact W8 K=512 path is fused with routed down, merge, and `AddResidual` in D4. Grouped performance remains unsupported. |
...
execution currently meets the support definition for the 35B target. Neither decode-only
`SparseMoe` support nor the full-domain functional fallback registers that target.
