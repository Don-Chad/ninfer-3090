...
| S3 | Gated delta-rule recurrence; BF16 Q/K `[128,16,T]`, V/O `[128,32,T]`, FP32 g/beta `[32,T]`, FP32 state `[128_v,128_k,32_heads]` | For V head j, `S_j` has axes `[Vdim,Kdim]` and uses Q/K head `floor(j/2)`: `Sbar=exp(g[j,t])*S_j`; `delta=beta[j,t]*(v-Sbar*k)`; `S_j'=Sbar+delta*k^T`; `o=(S_j'*q)/sqrt(128)`. Tokens update state in order. Ordinary in-place and distinct-state forms are required. | **Adapt existing.** Current recurrent/chunked Ops are fixed to 48 V heads and mapping `floor(j/3)`. |
| S4 | Snapshot form of S3 for `1<=T<=6`; FP32 state slots `[128_v,128_k,32_heads,7]`, I32 `initial_slot` in `[0,7)` | Reads the initial recurrent state from `initial_slot`; after column t, publishes the resulting state in slot t and leaves later slots unchanged. | **Adapt existing.** Snapshot machinery has the 27B state geometry. |
...
| Status | Exact result |
|---|---|
| **Supported** | Only N5 Vision LayerNorm `[1152,4096]`, E1 Vision QKV bias add `[3456,4096]`, E6 tanh GELU `[4304,4096]`, and A4 one-segment Vision attention `[72,16,4096]`. |
| **Adapt existing** | Every target-callable dense/quantized Linear domain plus private L15-L17; indexing transforms; all Text/GDN/MTP normalization; pointwise domains other than E1/E6; Text and Vision RoPE; all 35B GQA routes; GDN convolution/recurrence; generation and MTP-control Ops. All remain unsupported. |
...
No Text, GDN, sparse-MoE, generation, or MTP operation currently meets the support definition for
the 35B target.
