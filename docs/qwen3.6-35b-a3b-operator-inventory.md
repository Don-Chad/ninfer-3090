...
| A2 | KV append only; BF16 K/V `[256,2,T]`, I32 `cache_positions[T]`, both cache routes | Overwrites every addressed K/V value and scale without computing attention or advancing the host cursor. | **Adapt existing.** The exact two-KV-head append implementation exists and passes operation parity, but the standalone A2 route has not received retained fixed-resource or bandwidth qualification. |
...
| Status | Exact result |
|---|---|
| **Supported** | N5 Vision LayerNorm `[1152,4096]`, E1 Vision QKV bias add `[3456,4096]`, E6 tanh GELU `[4304,4096]`, A1 append-and-attend GQA `[256,16|2,T]`, A4 one-segment Vision attention `[72,16,4096]`, S1/S2 GDN causal convolution and snapshot state, and S3/S4 GDN recurrence and snapshot state transition `[128,16,32]`. |
| **Adapt existing** | Every target-callable dense/quantized Linear domain plus private L15-L17; indexing transforms; all Text/GDN/MTP normalization; pointwise domains other than E1/E6; Text and Vision RoPE; standalone GQA A2/A3; generation and MTP-control Ops. All remain unsupported. |
