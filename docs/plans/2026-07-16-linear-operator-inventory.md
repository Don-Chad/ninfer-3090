...
> 状态：当前源码现状清单与逐算子整理队列。尚未开始架构迁移，也未修改生产实现。
...
- 一批为死分支编译的专用 kernel；
- 主要由测试和测量使用的任意 shape dense/quantized 通用执行面。
...
|---|---|---|---|---|
| S1 | `linear` | 一个 BF16 `X[K,T]`、一个 dense 或 RowSplit `W[N,K]`，输出 BF16 `Y[N,T]` | dense reference GEMV/GEMM；通用低比特 Small-T；低比特 MMA；W8 MMA；精确 T1 GEMV | Text、MTP、Vision 全部可达 |
| S2 | `linear_pair` | 同一输入上的两个同 shape Q5 或 W8 投影 | T1 精确 Q5 dual GEMV；`T>16` W8 dual MMA；其他情况调用两次 public `linear()` | W8 `[1024,5120] x2` 可达；Q5 dual 路径不可达 |
...
| S5 | `linear_add` | 固定 Q5 投影后原地更新 BF16 residual | T1 精确融合；T2..16 显式中间张量加 residual；T17+ residual MMA | 两个问题，共调用 128 次/层遍历 |
...
### 4.2 Q4/Q5/Q6 RowSplit

| 问题范围 | 当前执行策略 | 重要的内部选择 |
|---|---|---|
| 已登记的精确 T1 shape | 9 个已调优 policy ID 之一 | 专用 shape kernel 或固定 Q5 core 实例 |
| 其他 T1 shape | `RowsplitLowbitGemmSmallt` | 通用 TT4 Small-T kernel |
| `T=2..16` | `RowsplitLowbitGemmSmallt` | 部分 Q5 shape/T2..6 走 direct kernel；其他走 TT4/TT8 |
| `T>16` 且 `K%8==0` | `RowsplitLowbitGemmMma` | shape-specific BN64 或默认 BN128，再选完整/边界 tile |
| `T>16` 且 `K%8!=0` | `RowsplitLowbitGemmSmallt` | 通用 TT8 兜底路径按 token tile 重读权重 |
...
- Q4 `[4096,5120]`；
- Q5 `[5120,17408]`；
- 固定 `K=5120`、运行时 N 的 Q4/Q6 vocab head；
- Q5 `[6144,5120]`；
- Q5 `[5120,6144]`。

Q5 Small-T 内核启动层对 `T=2..6` 还会继续精确分流：
...
- Q4 `[4096,5120]` 在 T128 及以下使用 BN64；
- Q5 `[5120,17408]`、`[5120,6144]` 在 T128 及以下使用 BN64；
- Q5 `[6144,5120]` 在 T64 及以下使用 BN64；
- 其他情况使用默认 BN128；
- 最后再选择完整 tile 或边界 tile 生成代码。
...
因此，当前 plan 仍不能完整确定 kernel；几何和边界选择继续隐藏在内核启动层中。
...
| RowSplit Small-T | `linear_rowsplit_gemm_smallt.cu/.cuh` | Q4/Q5/Q6/W8，任意 N/K，TT4/TT8，支持标量 K tail | 主要可复用 SIMT 基础 |
| Q5 direct Small-T | 同上 | 4 个精确 shape，T2..6，split-2/split-4 | 精确执行策略候选，不是通用 regime |
| Low-bit MMA | `linear_rowsplit_gemm_mma.cu/.cuh` | Q4/Q5/Q6，运行时 N/K/T，BN64/BN128，完整/边界 tile | 主要可复用 tensor-core 基座 |
| W8 MMA | `linear_rowsplit_w8g32_gemm_mma.cu/.cuh` | W8，运行时 N/K/T，small-M/通用配置，完整/边界 tile | 独立 W8 生命周期 |
| Runtime-N head GEMV | `linear_rowsplit_gemv_lm_head.cu` | Q4/Q6，固定 K5120，运行时 N | 范围较窄，但已在 vocab N 维度泛化 |

### 5.2 精确 T1 或拓扑专用 kernel

| 家族 | 精确问题 | 说明 |
|---|---|---|
| Q4 MLP gate/up GEMV | `[34816,5120]` | 普通写回和融合 SwiGLU 为不同 kernel |
| Q4 attention 父权重 GEMV | `[7168,5120]` | 6144-row 与 1024-row 使用混合几何；当前产品分支不可达 |
| Q4 GDN Q/K GEMV | `[4096,5120]` | 每行 8-way split K |
| 参数化 Q5 core | 多组固定 N/K | 普通写回、residual、dual-output 共用模板基座 |
| Q5 GDN V/Z dual GEMV | 两个 `[6144,5120]` 权重 | 当前产品分支不可达 |
...
- Q4/Q5/Q6/W8 decode codec；
- 一个通用 RowSplit Small-T 拓扑；
- 一个 Q4/Q5/Q6 MMA 拓扑；
...
- `linear_add` 在 `T=2..16` 显式写出 `[5120,T]` 中间张量；
...
当前 target 没有 `Phase::Decode` 调用者。`run_layers()` 只会收到 `Phase::Verify` 或
`Phase::Prefill`，因此两个 `Phase::Decode` 分支不能建立产品支持。

以下已调优执行路径已经编译，但产品 schedule 不可达：

- Q4 父权重 `[7168,5120]` T1 attention GEMV；
- Q5 父权重 `[7168,5120]` T1 attention GEMV；
...
以下源码可见执行策略可以通过仓库内部 API 调用，但不会被当前 target 选中：

- 普通写回 T1 Q4 `[34816,5120]`，因为 Text T1 使用融合 `linear_swiglu`；
- 普通写回 T1 Q5 `[5120,17408]` 和 `[5120,6144]`，因为 Text T1 使用融合
  `linear_add`；
- 单输出 W8 MMA small-M 配置：唯一可达的 W8 `N=1024` 问题是 B12，而其 `T>16`
  路径会被 paired W8 kernel 截获；
- `Generic` 接受的任意 dense 和 quantized shape。
...
- base planner 只命名一个执行策略，但 Small-T、low-bit MMA、W8 MMA、dense 内核启动层
  都会继续做行为选择；
- 融合语义 wrapper 各自重复相同的全局 `T=1/16` 阈值；
- 当前 benchmark 的 “all target” matrix 不等于真实可达精确执行问题清单；
...
| Base quantized Linear | Q4/Q5/Q6/W8 CPU dequant oracle；覆盖 T1/Small-T/Large-T 和 tail shape | 完整规模的精确 high-T Vision 问题缺独立 oracle |
| `linear_pair` | 小型 generic shape 的 W8 正确性 | 缺 B12 精确几何和全部路由边界 |
| 共享输入多投影算子 | Large-T 与多个 base Linear 输出比较 | 不是完全独立的数学 oracle |
| `linear_add` | T17/128/129 与 Linear 加 residual 比较 | 未直接保留 T1 和显式中间张量 Small-T 语义边界测试 |
| `linear_swiglu` | T17/128/129 与 Linear 加 `silu_mul` 比较 | 未直接保留 T1 和显式中间张量 Small-T 语义边界测试 |
...
| 二进制 | 有效覆盖 | 清单问题 |
|---|---|---|
| `ninfer_linear_bench` | dense 和部分 decode 的快速信号 | shape 标签包含旧的子视图几何，不能代表完整登记域 |
| `ninfer_linear_op_bench` | base Linear、W8 pair 的 cold/warm 测量与 roofline probe | `--all-targets` 只有 17 行，包含不可达父权重和非产品 draft 变体，遗漏有效 row view 和 Vision |
...
当前树没有 proposed Op-owned exact catalog、闭合路由区间或固定的显式中间张量 subplan
的测试。历史结构测试只存在于归档实验分支，必须按照新的所有权和 token 支持范围重写。
