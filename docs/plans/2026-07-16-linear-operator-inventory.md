...
Q5/Q6 尚未迁移，继续使用旧 planner：

| 问题范围 | 当前执行策略 | 重要的内部选择 |
|---|---|---|
| 已登记的精确 T1 shape | 已调优 Q5/Q6 policy ID | 专用 shape kernel 或固定 Q5 core 实例 |
...
| `T>16` 且 `K%8==0` | `RowsplitLowbitGemmMma` | Q5 shape-specific BN64 或默认 BN128，再选完整/边界 tile |
| `T>16` 且 `K%8!=0` | `RowsplitLowbitGemmSmallt` | 通用 TT8 路径按 token tile 重读权重 |

当前专用 T1 执行策略包含 Q5 `[7168,5120]`、Q5 `[5120,17408]`、固定 K5120
运行时 N 的 Q6 vocab head、Q5 `[6144,5120]` 和 Q5 `[5120,6144]`。
...
| RowSplit Small-T | `linear_rowsplit_gemm_smallt.cu/.cuh` | Q5/Q6/W8，任意 N/K，TT4/TT8，支持标量 K tail | 尚未迁移格式的 SIMT 基础 |
| Q5 direct Small-T | 同上 | 4 个精确 shape，T2..6，split-2/split-4 | 精确执行策略候选，不是通用 regime |
| Low-bit MMA | `linear_rowsplit_gemm_mma.cu/.cuh` | Q5/Q6，运行时 N/K/T，BN64/BN128，完整/边界 tile | grouped/fused 仍复用其 header 基座 |
| W8 MMA | `linear_rowsplit_w8g32_gemm_mma.cu/.cuh` | W8，运行时 N/K/T，small-M/通用配置，完整/边界 tile | 独立 W8 生命周期 |
| Runtime-N head GEMV | `linear_rowsplit_gemv_lm_head.cu` | Q6，固定 K5120，运行时 N | 范围较窄，但已在 vocab N 维度泛化 |
...
Q5 父权重 `[7168,5120]` T1 GEMV 与 Q5 dual `[6144,5120]` V/Z T1 GEMV 仍已编译但
产品 schedule 不可达；它们属于后续 Q5 审查，不是本轮 Q4 清理范围。
...
- 旧 planner 的 `Generic` 仍可接受部分尚未迁移的 dense/Q5/Q6/W8 shape。
