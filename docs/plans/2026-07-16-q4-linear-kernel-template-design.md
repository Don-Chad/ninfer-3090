...
> 状态：详细设计、dormant candidate、route 资格与 NCU 分析已完成；production 原子切换
> 进行中。
>
...
4. 独立 legacy measurement selector；
5. canonical schedule、variant、build type、CUDA 和 GPU CSV 字段；
6. GEMV/SIMT useful-byte 与 MMA executed-FLOP 的分离统计；
7. `--help`、smoke CSV 和 candidate legality 验证。
