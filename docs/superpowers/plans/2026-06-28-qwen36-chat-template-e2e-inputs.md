...
- [ ] Do not add a general runtime JSON dependency unless the repo already has one. Prefer a small benchmark-local parser because the manifest shape is narrow and generated by our Python tooling.
...
- [ ] If a full mini JSON parser is too large for one file, scope it explicitly to objects, arrays, strings, booleans, integers, and null. The manifest does not require floats.

- [ ] Do not parse JSON with ad hoc substring matching for fields that can appear in nested objects with the same names. The parser may be small, but it must be structurally aware.
