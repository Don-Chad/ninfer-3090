...
| `--kv-dtype bf16\|int8\|rk8v4\|fp8` | KV-cache storage; `rk8v4` is experimental and lossy, `fp8` is row-scaled E4M3 | `bf16` |
...
depends on the GPU, the selected artifact, media workload, output budget, and KV-cache type.
Use `--kv-dtype int8` for the recommended large-context quality profile. `--kv-dtype fp8`
selects upstream's row-scaled E4M3 D256 KV storage. Experimental `rk8v4` stores rotated values
at four bits and keeps rotated keys at eight bits; it maximizes capacity on 24 GB cards but is
not quality-equivalent to INT8. Artifact identity selects the weight profile; `--kv-dtype`
