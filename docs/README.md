...
| [`ninfer-engine-architecture.md`](ninfer-engine-architecture.md) | accepted future NInfer core engine boundary: compiled exact-target packages, load-time construction, ownership and memory lifetimes, checkpoint frontends, one-request program state, transactional generated-token rounds, and common-versus-target-private interfaces; no current implementation, model math, format details, serving protocol, migration plan, or future scheduler authority |
